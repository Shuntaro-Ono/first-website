...
トップページでは「名前」→「レターボックス番号」の順に入力してもらいます。
手紙が開くのは **入力された名前が `LETTERS` のキーと完全一致し、かつ入力された
番号がそのキーの `box` の値とも一致したとき** だけです（名前だけ／番号だけ合って
いても開きません）。今のデータでは4人とも `box` が `"101"` なので、全員に
「レターボックス番号は101だよ」と伝えれば大丈夫です。今後、人によって番号を
変えたくなったら `box` の値を個別に変更してください。

## 中身を更新したいとき

コードを触る必要はありません。`data.js` を開いて：

- **手紙を追加・編集する** → `LETTERS` オブジェクトに、相手が入力する名前を
  キーにして `box` / `date` / `body`（段落ごとに配列）/ `signature` を書く
  （既存の4件の書き方をそのまま真似すればOKです）
- **棚の写真を追加する** → `images/` フォルダに指定のファイル名で画像を置くだけ
  （置かなければ絵文字のプレースホルダーが表示されます）
- **郵便ボックスの番号の見た目一覧を変える** → `POST_BOX_NUMBERS` の配列を編集
  （POST BOXESセクションの表示だけで、実際にどの番号で開くかは各手紙の
  `box` の値で決まります）
